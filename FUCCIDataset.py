...
class FUCCIDataset(Dataset):
    ...
    """
    def __init__(self, data_dir, rebuild=False):
        ...
        for split in self.data_dir.iterdir():
            if not split.is_dir():
                raise ValueError(f"{split} is not a directory")

            for experiment in split.iterdir():
                if not experiment.is_dir():
                    raise ValueError(f"{experiment} is not a directory")
    ...
    def __image_from_experiment(self, experiment_dir):
        ...
        def channel_from_dir(names, dir):
            return list(filter(lambda x: any(name in x.stem.lower() for name in names), dir.iterdir()))[0]
    ...
    def __getitem__(self, idx):
        ...
        return np.roll(cell_image, offset, axis=(0, 1))
...
def clean_dir(data_dir):
    ...
    for split in data_dir.iterdir():
        for experiment in split.iterdir():
            ...
            if experiment / "com.pkl" in experiment.iterdir():
                os.remove(experiment / "com.pkl")
...
if __name__ == "__main__":
    ...
    dataset = FUCCIDataset(dataset_location)
