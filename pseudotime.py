...
class FucciCellCycle:
    def __init__(self):
        ...
        self.G1_LEN = 10.833  # hours 
        self.G1_S_TRANS = (
            2.625  # hours plus 10.833, so 13.458hrs for the S/G2 cutoff
        )
        self.S_G2_LEN = ( # this should be from the G2/M cutoff above to the end
            11.975  # hours plus 10.833 and 2.625 so 25.433 hrs for the G2/M cutoff
        )
...
def mvavg(yvals, mv_window):
    ...
    # valid makes it so the result is len(yvals) - mv_window + 1
    # this is so only points that have a full window are included
...
def frange(start, stop, step):
    ...
    while i < stop:
        yield i
        # why are we rounding to 14 decimal places?
        i = round(i + step, 14)
